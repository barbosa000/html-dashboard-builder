...
function ProducaoPage() {
  ...
  const preview = bagsFromKg(
    Math.max(0, parseFloat(form.kg) || 0),
    Math.max(0, parseFloat(form.mix) || 0),
  );
  ...
  return (
    <div>
      <PageHeader
        title="Produção"
        description="Lançamento diário de produção de gelo e acompanhamento da meta mensal."
      />

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Kpi
          label="Kg produzidos (mês)"
          value={numFmt(totalKg, 1)}
          sub={`Meta: ${numFmt(monthGoal, 0)} kg`}
        />
        ...
        <Kpi
          label="Dias lançados"
          value={numFmt(daysLancados)}
          sub={`de ${settings.daysPerMonth || 26} dias úteis`}
        />
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Panel
          ...
          subtitle="Lançar de novo na mesma data pode ser usado para corrigir o registro (edite pela tabela)."
        >
          <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                required
                value={form.kg}
                onChange={(e) => setForm((s) => ({ ...s, kg: e.target.value }))}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                step="1"
                value={form.mix}
                onChange={(e) => setForm((s) => ({ ...s, mix: e.target.value }))}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                step="0.1"
                value={form.perda}
                onChange={(e) => setForm((s) => ({ ...s, perda: e.target.value }))}
              />
            </div>
        ...
        <Panel title="Produção — últimos 14 dias" subtitle="Kg produzidos por dia">
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <Tooltip
                  contentStyle={{
                    background: "var(--surface-solid, #101828)",
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Capacidade nominal da máquina: {numFmt(cap)} kg/dia.
          </p>
        </Panel>
      </div>
          ...
          return (
            <tr key={p.id} className="hover:bg-surface-2/50">
              ...
              <Td>
                <span
                  className={
                    eff >= 90 ? "text-success" : eff >= 60 ? "text-warning" : "text-destructive"
                  }
                >
                  {numFmt(eff, 0)}%
                </span>
