...
import { PageHeader, Panel, Kpi, Chip, DataTable, Td, SectionTitle, Note } from "@/components/ui-kit";
...
function MovementsForm({ supplies }: { supplies: any[] }) {
  ...
  const { rows, add, remove } = useCollection("movements");
  const [itemId, setItemId] = useState("");
  const [tipo, setTipo] = useState("entrada");
  const [qtd, setQtd] = useState("");
  const [date, setDate] = useState(todayStr());

  const sorted = useMemo(() => [...rows].sort((a, b) => String(b.date).localeCompare(String(a.date))).slice(0, 50), [rows]);
    ...
    <Panel title="Movimentações de estoque">
      <form onSubmit={submit} className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        <div className="col-span-2 space-y-1.5">
          ...
          <select className="h-10 w-full rounded-md border border-input bg-surface-2 px-3 text-sm" value={itemId} onChange={(e) => setItemId(e.target.value)}>
        ...
        <div className="space-y-1.5">
          ...
          <select className="h-10 w-full rounded-md border border-input bg-surface-2 px-3 text-sm" value={tipo} onChange={(e) => setTipo(e.target.value)}>
        ...
        <div className="space-y-1.5">
          ...
          <Input type="number" min={0} step="0.01" value={qtd} onChange={(e) => setQtd(e.target.value)} />
        </div>
        <div className="space-y-1.5">
      ...
      <div className="mt-5">
        <DataTable columns={["Data", "Item", "Tipo", "Qtd.", ""]} isEmpty={sorted.length === 0} empty="Nenhuma movimentação registrada.">
          {sorted.map((m) => (
            <tr key={m.id} className="hover:bg-surface-2/50">
              <Td>{fmtDateBR(m.date)}</Td>
              <Td>{m.itemName || "—"}</Td>
              <Td>
                <Chip value={m.type === "entrada" ? "Entrada" : "Saída"} kind={m.type === "entrada" ? "good" : "critical"} />
              </Td>
...
function EstoquePage() {
  ...
  return (
    <div>
      <PageHeader title="Estoque" description="Estoque de gelo estimado e controle de insumos/embalagens." />

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Kpi label="Estoque sacos 2 kg" value={`${numFmt(inv.estQ2)} un.`} sub={`${numFmt(inv.estQ2 * 2)} kg equivalentes`} tone={inv.estQ2 < 0 ? "critical" : "default"} />
        <Kpi label="Estoque sacos 5 kg" value={`${numFmt(inv.estQ5)} un.`} sub={`${numFmt(inv.estQ5 * 5)} kg equivalentes`} tone={inv.estQ5 < 0 ? "critical" : "default"} />
      </div>

      <div className="mt-4">
        <Note kind="info">
          Estoque de gelo calculado automaticamente: total produzido (por formato) menos vendas registradas com status diferente de "Cancelado".
        </Note>
      </div>
      ...
      <SectionTitle hint="🟢 normal · 🟡 baixo · 🔴 crítico">Embalagens, matéria-prima e insumos</SectionTitle>
      <CrudSection
        collection="supplies"
        ...
        fields={[
          ...
          { key: "category", label: "Categoria", type: "select", options: ["Embalagens", "Matéria-prima", "Insumos"] },
        ...
        columns={[
          ...
          { key: "quantity", label: "Quantidade", render: (r) => `${numFmt(r.quantity, 2)} ${r.unit || ""}` },
          {
            key: "status",
            label: "Status",
            render: (r) => {
              const st = supplyStatus(r);
              return <Chip value={st} kind={st === "Normal" ? "good" : st === "Baixo estoque" ? "warn" : "critical"} />;
            },
          },
        ]}
      />
